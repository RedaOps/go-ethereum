...
func (api *API) TraceVictimArbTransactionsRetdata(ctx context.Context, victimTx hexutil.Bytes, arbTxes []hexutil.Bytes, blockNrOrHash rpc.BlockNumberOrHash) ([]*core.ExecutionResult, error) {
	vTx := new(types.Transaction)
	if err := vTx.UnmarshalBinary(victimTx); err != nil {
		return nil, err
	}
